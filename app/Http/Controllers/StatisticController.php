...
class StatisticController extends Controller
{
    public function getDoanhThu(Request $request){
        ...
        $thongke=DB::table("bills")->select(DB::raw("MONTH(date_order) as Thang")  ,DB::raw("(SUM(total)) as Tong"))->groupBy(DB::raw("MONTH(date_order)"))->where('tinhtrang',1)->whereYear('date_order', $year)->get();
